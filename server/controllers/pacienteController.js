...
const createPaciente = async (req, res) => {
    ...
    try {
        const paciente = await Paciente.create({ nombre, dni, tutor_id: tutor_id || null });
...
const updatePaciente = async (req, res) => {
    const { id } = req.params;
    const { nombre, dni, tutor_id } = req.body;
    try {
        const paciente = await Paciente.findByPk(id);
        if (!paciente) {
            return res.status(404).json({ success: false, message: "Paciente no encontrado" });
        }
        await paciente.update({ nombre, dni, tutor_id: tutor_id || null, });
